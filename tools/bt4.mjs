...
await new Promise(r=>s.listen(4568,r));
...
// disable JS to isolate CSS
await pg.goto("http://localhost:4568/pages/pricing.html",{waitUntil:"load"});
await pg.waitForTimeout(300);
const trace=await pg.evaluate(async()=>{const sc=document.querySelector('.story-scroll');const out=[];for(let i=0;i<15;i++){const before=sc.scrollTop;sc.scrollBy(0,400);await new Promise(r=>setTimeout(r,150));out.push(Math.round(sc.scrollTop));}return out;});
console.log("JS-on trace:",JSON.stringify(trace));
