...
await new Promise(r=>s.listen(4561,r));
...
const pg=await b.newPage({viewport:{width:1440,height:400}});
await pg.goto("http://localhost:4561/",{waitUntil:"load"});
await pg.waitForTimeout(700);
// dismiss cookie banner if present
await pg.evaluate(()=>{const c=document.querySelector("[class*=cookie] button, .cookie-close, #cookie-close");});
await pg.screenshot({path:SC+"pill-fixed.png"});
