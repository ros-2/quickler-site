...
await new Promise(r=>s.listen(4510,r));
try{
  ...
  for(const [path,name] of [["/pages/services.html","SERVICES"],["/pages/about.html","ABOUT"],["/pages/eicr-software-uk.html","GUIDE"]]){
    const pg=await b.newPage({viewport:{width:1440,height:1000}});
    await pg.goto("http://localhost:4510"+path,{waitUntil:"load",timeout:20000});
    ...
    await pg.screenshot({path:"tools/shots/"+name+".png",fullPage:true});
