...
try{
  const b=await chromium.launch({executablePath:"/home/balnagowan/.cache/ms-playwright/chromium_headless_shell-1228/chrome-headless-shell-linux64/chrome-headless-shell-linux64/chrome-headless-shell".replace("chrome-headless-shell-linux64/chrome-headless-shell-linux64","chrome-headless-shell-linux64"),args:["--no-sandbox","--disable-gpu"]});
  ...
  // top of page: nav un-compacted, WhatsApp glyph visible
  await pg.screenshot({path:"tools/shots/HOME-TOP.png",clip:{x:0,y:0,width:1440,height:600}});
  // scroll to the feature band (the shift section) + compacted nav
  await pg.evaluate(()=>{const el=document.querySelector(".story-feature");if(el)el.scrollIntoView();});
  await pg.waitForTimeout(700);
  await pg.screenshot({path:"tools/shots/HOME-FEATURE.png",clip:{x:0,y:0,width:1440,height:700}});
