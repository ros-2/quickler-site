...
await new Promise(r=>s.listen(4557,r));
...
const pg=await b.newPage({viewport:{width:1900,height:950}});
await pg.goto("http://localhost:4557/",{waitUntil:"load"});
await pg.waitForTimeout(600);
const d=await pg.evaluate(()=>{const n=document.querySelector("nav");const cs=getComputedStyle(n);const root=getComputedStyle(document.documentElement);return {width:Math.round(n.getBoundingClientRect().width),vw:window.innerWidth,radius:cs.borderTopLeftRadius,cssWidth:cs.width,contentWidth:root.getPropertyValue('--content-width'),widthPage:root.getPropertyValue('--width-page'),left:Math.round(n.getBoundingClientRect().left)};});
console.log("AT 1900 (top):",JSON.stringify(d,null,2));
